...
foreach ($user in $usersRaw) {
    ...
    try {
        New-ADUser `
            ...
            # Add non-standard attribute values to the user's extension attributes.
            $otherAttributes = @{}
            ...
            # Then include it only if there's anything to set
            if ($otherAttributes.Count -gt 0) {
                Set-ADUser -Identity $sam -Add $otherAttributes
            }
